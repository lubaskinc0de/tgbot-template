...
async def create_item(session: Session, shops: list[tuple[int, int]], photos: list[str], **kwargs: dict) -> None:
    '''
    ...
    **kwargs are directly instance fields without shops
    ...
    item_category_id = kwargs.pop('category_id')
    category = await get_category(session, item_category_id)
...
async def get_item_shops(session: Session, item_id: int, show_empty: bool = False) -> list[ItemShop]:
    '''
    Get item shops
    ...
    q = select(ItemShop).where(ItemShop.item_id == item_id).options(joinedload(ItemShop.shop))
...
async def get_item_shops_count(session: Session, item_id: int, show_empty: bool = False) -> list[ItemShop]:
    '''
...
async def get_item_shop(session: Session, item_id: int, shop_id: int) -> ItemShop:
    ...
    q = select(ItemShop).where(ItemShop.shop_id == shop_id).where(ItemShop.item_id == item_id)
    res = await session.execute(q)
...
async def set_item_shop_quantity(session: Session, item_id: int, shop_id: int, quantity: int) -> None:
