...
from db.services.service_categories import get_service_categories, get_service_categories_count
from db.services.shops import get_shops, get_shops_count
from db.services.items import get_item_shops, get_item_shops_count, get_items_by_category, get_items_by_category_count
...
async def get_categories_data(db_session: Session, **kwargs):
    ...
    categories = [
        ...
        for category in await get_categories(db_session)
    ]
...
async def get_service_categories_data(db_session: Session, **kwargs):
    ...
    service_categories = [
        (service_category.title, service_category.id)
        for service_category in await get_service_categories(db_session)
    ]
...
async def get_shops_data(db_session: Session, **kwargs):
    ...
    shops = [
        (shop.title, shop.id)
        for shop in await get_shops(db_session)
    ]
...
async def get_item_shops_data(db_session: Session, dialog_manager: DialogManager, **kwargs):
    ...
    item_shops = [
        ...
        for item_shop in await get_item_shops(db_session, item_id)
    ]
...
async def get_category_items_data(db_session: Session, dialog_manager: DialogManager, **kwargs):
    ...
    category_items = [
        (item.title, item.id)
        for item in await get_items_by_category(db_session, category_id)
    ]
