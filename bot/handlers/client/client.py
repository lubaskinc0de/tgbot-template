...
start_window = Window(
    ...
    Row(
        ...
        Start(Const('Заказать услугу'), id='service_order', state=CreateServiceOrderSG.set_service_category),
    ),
    Button(Const('Купленные товары'), id='user_items', on_click=user_items),
    state=ClientSG.start,
    )

async def start(message: types.Message, dialog_manager: DialogManager, db_session: Session):
    '''
