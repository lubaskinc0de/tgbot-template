...
async def item_shops(call: types.CallbackQuery, widget: Any, manager: DialogManager):
    ...
    await manager.start(ItemShopsSG.item_shops, {
        'item_id': manager.start_data.get('item_id')
    })

async def item_order_create(call: types.CallbackQuery, widget: Any, manager: DialogManager):
    '''Item order create'''

    await manager.start(CreateItemOrderSG.create_item_order, {
        'item_id': manager.start_data.get('item_id')
    })
...
async def item_photos(call: types.CallbackQuery, widget: Any, manager: DialogManager):
    ...
    item_photos: list[types.InputMediaPhoto] = [types.InputMediaPhoto(media=photo.file_id) for photo in item.photos]
