...
async def get_order_data(dialog_manager: DialogManager, bot: Bot, **kwargs):
    '''service_order_created_window data getter'''

    order_id: int = dialog_manager.dialog_data.get('order_id')

    admins = await get_admins(bot)
...
async def set_service_description(message: types.Message, widget: Any, manager: DialogManager, input: str):
    ...
    try:
        ...
        service_id = (await create_service(db_session, service.category_id, **service.dict(exclude={'category_id'}))).id

        user_id: int = message.from_user.id

        order = OrderModel(service_id=service_id, user_id=user_id, **manager.dialog_data)

        order = await create_order(manager.middleware_data.get('db_session'), **order.dict())
...
async def set_service_title(message: types.Message, widget: Any, manager: DialogManager, input: str):
    '''Set service title'''
...
async def set_service_category(call: types.CallbackQuery, widget: Any, manager: DialogManager, category_id: str):
...
create_service_order_dialog = Dialog(set_service_category_window, set_service_title_window, set_service_description_window, service_order_created_window)