...
async def get_user_items_data(db_session: Session, dialog_manager: DialogManager, **kwargs):
    '''User items data getter'''

    user_id: int = dialog_manager.start_data.get('user_id')

    user_items = [
        (user_item.item.title, user_item.item_id)
        for user_item in await get_user_items(db_session, user_id)
    ]
...
async def user_items(message: types.Message, widget: Any, dialog_manager: DialogManager):
    '''
    Get user items
    '''
    ...
    await dialog_manager.start(UserItemsSG.user_items, {
        'user_id': user_id
    })