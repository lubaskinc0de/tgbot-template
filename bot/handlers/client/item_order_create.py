...
async def set_order_quantity(message: types.Message, widget: Any, manager: DialogManager, quantity: str):
    ...
    if quantity > item_in_shop_quantity:
        await message.answer('Вы ввели кол-во больше чем есть в магазине. Повторите попытку.')
        return
    try:
        item_id: int = manager.start_data.get('item_id')
        ...
        order = OrderModel(item_id=item_id, user_id=user_id, summ=order_summ, **manager.dialog_data)
...
async def set_order_shop(call: types.CallbackQuery, widget: Any, manager: DialogManager, shop_id: str):
    '''Set order shop'''
...
create_item_order_window = Window(
    ...
    SwitchTo(Const('Продолжить?'), id='set_order_shop_quantity', state=CreateItemOrderSG.set_order_shop),
...
create_item_order_dialog = Dialog(create_item_order_window, set_order_shop_window, set_order_quantity_window, item_order_created_window)