...
from aiogram_dialog.widgets.kbd import Cancel, Next, ScrollingGroup, Select, Button, SwitchTo
...
async def set_item_shops(message: types.Message, widget: Any, manager: DialogManager):
    ...
    manager.dialog_data['shops'] = list(map(lambda shop: tuple(map(int, shop)), selected_shops.items()))

    try:
        item = ItemModel(**manager.dialog_data)

        await create_item_service(
            manager.middleware_data.get('db_session'), item.shops, item.photos, **item.dict(exclude={'shops', 'photos'})
            )
...
async def set_item_category_id(message: types.Message, widget: Any, manager: DialogManager, category_id: int):
...
async def set_item_price(message: types.Message, widget: Any, manager: DialogManager, input: str):
    '''Set item price'''
...
set_item_price_window = Window(
    Const('Введите цену товара, не больше 12 цифр в числе и не больше 2 цифр после запятой.\nПример: 9999999999.99'),
...
async def set_item_description(message: types.Message, widget: Any, manager: DialogManager, input: str):
...
async def set_item_title(message: types.Message, widget: Any, manager: DialogManager, input: str):
    '''Set item title'''
...
create_item_dialog = Dialog(
    start_create_item_window, set_item_description_window, set_item_photos_window,
    set_item_price_window, set_item_category_id_window, set_item_shops_window,
    )