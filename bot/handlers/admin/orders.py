...
async def get_order_details(call: types.CallbackQuery, widget: Any, manager: DialogManager, order_id: int):
    '''Get order details'''

    await manager.start(OrderSG.order_details, {
        'order_id': int(order_id)
    })

async def get_orders_data(db_session: Session, bot: Bot, **kwargs):
    '''Orders data getter'''
