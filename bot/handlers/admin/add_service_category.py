...
from db.services.service_categories import create_service_category as create_service_category_service

from sqlalchemy.exc import DBAPIError

from dialog.dialog_state import CreateServiceCategorySG

async def create_service_category_success(message: types.Message, widget: Any, manager: DialogManager, input: str):
    '''Create service category success'''

    try:
        service_category = ServiceCategoryModel(title=input)
        await create_service_category_service(manager.middleware_data.get('db_session'), **service_category.dict())
