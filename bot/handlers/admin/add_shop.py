...
async def set_shop_closing_in(message: types.Message, widget: Any, manager: DialogManager, input: str):
    ...
    try:
        ...
        await create_shop_service(manager.middleware_data.get('db_session'), **shop.dict())
...
async def set_shop_opening_in(message: types.Message, widget: Any, manager: DialogManager, input: str):
    '''Set shop opening_in'''
...
async def set_shop_phone(message: types.Message, widget: Any, manager: DialogManager, input: str):
    '''Set shop phone'''
...
set_shop_phone_window = Window(
    Const('Введите телефон для связи с магазином, не больше 12 символов.\nПример: +7333333333'),
    TextInput('shopphoneinp', str, on_success=set_shop_phone),
    state=CreateShopSG.set_shop_phone
)

async def set_shop_address(message: types.Message, widget: Any, manager: DialogManager, input: str):
    '''Set shop address'''
...
async def set_shop_title(message: types.Message, widget: Any, manager: DialogManager, input: str):
    '''Set shop title'''
...
create_shop_dialog = Dialog(
    start_create_shop_window, set_shop_address_window, set_shop_phone_window, set_shop_opening_in_window, set_shop_closing_in_window
    )
