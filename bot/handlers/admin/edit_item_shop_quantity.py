...
from db.services.items import get_item_shops, get_item_shops_count, set_item_shop_quantity as set_item_shop_quantity_service

from sqlalchemy.exc import DBAPIError
from sqlalchemy.orm import Session

async def set_item_shop_quantity(message: types.Message, widget: Any, manager: DialogManager, input: str):
    '''Set shop_id'''
...
async def set_shop_id(call: types.CallbackQuery, widget: Any, manager: DialogManager, shop_id: str):
    '''Set shop_id'''
...
async def get_item_shops_data(db_session: Session, dialog_manager: DialogManager, **kwargs):
    '''Item shops data getter'''

    item_id: int = dialog_manager.dialog_data.get('item_id')

    item_shops = [
        (item_shop.shop.title, item_shop.shop.address, item_shop.shop_id, item_shop.quantity)

        for item_shop in await get_item_shops(db_session, item_id, True)
    ]
...
async def list_of_item_shops(call: types.CallbackQuery, widget: Any, manager: DialogManager, item_id: str):
...
async def list_of_category_items(call: types.CallbackQuery, widget: Any, manager: DialogManager, category_id: str):
...
edit_item_shop_quantity_dialog = Dialog(categories_window, select_item_window, item_shops_window, set_item_shop_quantity_window)